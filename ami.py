...
def self_energy_denominator(omega, Delta, h_Sc):
    """
    Denominator of superconducting leads self-energy.
    """
    return np.sqrt(Delta ** 2 - (h_Sc - omega) ** 2)

def symbolic_hamiltonian_spinful(dims : int, self_energy : bool):
    """
    ...
    if self_energy:
        Sigma = sympify(
            ...
            gamma_Sc(x, y) * ( 
              ...
              (kron(sigma_0, sigma_0) + kron(sigma_0, sigma_z))/2 * self_energy_denominator(
               omega + 1j * Gamma_Sc, 
               Delta(x, y),
               +h_Sc)
            + (kron(sigma_0, sigma_0) - kron(sigma_0, sigma_z))/2 * self_energy_denominator(
